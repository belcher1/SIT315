...
// Set PIN for PID sensors
const uint8_t PID_PIN = 2;
const uint8_t PID2_PIN = 5;

// Variables for reading the PID state
volatile int PIDState = 0;
volatile int PID2State = 0;
...
void setup()
{
  ...
  // Set PID 1 pin to input & attach interrupt
...
ISR(PCINT2_vect)
{
  Serial.println("PID 2 On / Off");
  
  // Read the state of the PID sensor
  PID2State = digitalRead(PID2_PIN);
  // Change the state of the LED
  digitalWrite(LED3_PIN, PID2State);
}
